...
ValidationTest::SummaryStats ValidationTest::compute_summary_stats(const std::vector<double>& values)
{
    SummaryStats stats;
...
void ValidationTest::write_raw_runs_csv(const std::string& filepath, const std::vector<RunMetrics>& runs)
{
    ...
    {
        out << (i + 1) << "," << runs[i].totalTimeMs << "," << runs[i].avgTimePerStepMs << "," << runs[i].finalMassError
            << "," << runs[i].lastCellsModified << "\n";
    }
}

void ValidationTest::write_summary_csv(const std::string& filepath, const SummaryStats& totalStats,
                                       const SummaryStats& avgStepStats, const SummaryStats& massErrorStats,
                                       const SummaryStats& lastCellsStats)
{
    std::ofstream out(filepath);
    out << "metric,n,mean,median,stddev,min,max,ci95_low,ci95_high\n";

    out << "total_time_ms," << totalStats.n << "," << totalStats.mean << "," << totalStats.median << ","
        << totalStats.stddev << "," << totalStats.min << "," << totalStats.max << "," << totalStats.ci95Low << ","
        << totalStats.ci95High << "\n";

    out << "avg_time_per_step_ms," << avgStepStats.n << "," << avgStepStats.mean << "," << avgStepStats.median << ","
        << avgStepStats.stddev << "," << avgStepStats.min << "," << avgStepStats.max << "," << avgStepStats.ci95Low
        << "," << avgStepStats.ci95High << "\n";

    out << "final_mass_error," << massErrorStats.n << "," << massErrorStats.mean << "," << massErrorStats.median << ","
        << massErrorStats.stddev << "," << massErrorStats.min << "," << massErrorStats.max << ","
        << massErrorStats.ci95Low << "," << massErrorStats.ci95High << "\n";

    out << "last_cells_modified," << lastCellsStats.n << "," << lastCellsStats.mean << "," << lastCellsStats.median
        << "," << lastCellsStats.stddev << "," << lastCellsStats.min << "," << lastCellsStats.max << ","
        << lastCellsStats.ci95Low << "," << lastCellsStats.ci95High << "\n";
}

void ValidationTest::run_variant_tests(std::unique_ptr<Terrain>& terrain, const std::vector<float>& referenceData,
                                       const std::string& terrainType, int steps, ThermalVariant variant,
                                       NeighborhoodMode neighborhood)
{
    ...
    fs::path baseDir = fs::path("./resultat") / terrainType / neighborhoodName / variantName;
    ...
    write_summary_csv((baseDir / "summary_stats.csv").string(), totalStats, avgStepStats, massErrorStats,
                      lastCellsStats);
    ...
    std::cout << "95% CI total time (ms)     : [" << totalStats.ci95Low << ", " << totalStats.ci95High << "]\n";
...
void ValidationTest::run_all_tests(std::unique_ptr<Terrain>& terrain, const std::string& terrainType, int steps)
{
    ...
    const ThermalVariant baseVariants[] = {ThermalVariant::PureTwoPhase, ThermalVariant::BlockedPureTwoPhase,
                                           ThermalVariant::BlockedParallelPureTwoPhase};

    for (ThermalVariant variant : baseVariants)
    {
        run_variant_tests(terrain, referenceData, terrainType, steps, variant, NeighborhoodMode::EightNeighbors);
    }

    const ThermalVariant fourNeighborVariants[] = {ThermalVariant::PureTwoPhase,
    ...
    {
        run_variant_tests(terrain, referenceData, terrainType, steps, variant, NeighborhoodMode::FourNeighbors);
    }
}
