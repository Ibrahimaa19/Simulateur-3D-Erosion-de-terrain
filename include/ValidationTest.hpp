...
class ValidationTest
{
  ...
  private:
    ...
    static void write_raw_runs_csv(const std::string& filepath, const std::vector<RunMetrics>& runs);

    static void write_summary_csv(const std::string& filepath, const SummaryStats& totalStats,
                                  const SummaryStats& avgStepStats, const SummaryStats& massErrorStats,
                                  const SummaryStats& lastCellsStats);

    static void run_variant_tests(std::unique_ptr<Terrain>& terrain, const std::vector<float>& referenceData,
